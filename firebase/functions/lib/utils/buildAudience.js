...
export async function buildAudience(filters) {
    ...
    // Normalizar
    const country = typeof safe.country === "string" ? safe.country.trim() : "";
    const city = typeof safe.city === "string" ? safe.city.trim() : "";
    const role = typeof safe.role === "string" ? safe.role.trim() : "";
    const vip = typeof safe.vip === "boolean" ? safe.vip : undefined;
    const marketing = safe.marketing === true ? true : undefined; // ✅ solo filtra si es true
    const inactiveDays = typeof safe.inactiveDays === "number" && safe.inactiveDays > 0
        ? safe.inactiveDays
        : 0;
    ...
    async function runQuery(field) {
        ...
        if (useVip)
            query = query.where("vipFlag", "==", vip);
        if (useMarketing)
            query = query.where("notifPrefs.marketing", "==", true);
        if (useInactive) {
