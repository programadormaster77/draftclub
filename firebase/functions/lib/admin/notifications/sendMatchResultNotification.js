...
export const sendMatchResultNotification = onRequest(async (req, res) => {
    try {
        ...
        for (const uid of winners) {
            batch.set(db.collection("users").doc(uid).collection("matchResults").doc(roomId), { ...matchHistory, result: "victory" });
        }
        // Historial por usuario — perdedores
        for (const uid of losers) {
            batch.set(db.collection("users").doc(uid).collection("matchResults").doc(roomId), { ...matchHistory, result: "defeat" });
        }
        // ------------------------------------------------------------
