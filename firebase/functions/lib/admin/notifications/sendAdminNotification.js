...
// ============================================================================
export const sendAdminNotification = onRequest(async (req, res) => {
    try {
        // ----------------------------------------------------------------------
        ...
        const body = data?.body && typeof data.body === "string"
            ? data.body.trim()
            : "";
        const imageUrl = data?.imageUrl && typeof data.imageUrl === "string"
            ? data.imageUrl.trim()
            : null;
        const deepLink = data?.deepLink && typeof data.deepLink === "string"
            ? data.deepLink.trim()
            : null;
        // ✅ Conversión segura (sin undefined ni null directos)
        const country = data?.country !== undefined && data.country !== null
        ...
        console.log(`✅ Notificación enviada correctamente — Éxitos: ${result.successCount}, Fallos: ${result.failureCount}`);
        // ----------------------------------------------------------------------
        ...
        res.status(500).json({
            error: {
                message: error?.message || "Error desconocido en sendAdminNotification.",
                status: "INTERNAL",
            },
        });
    }
